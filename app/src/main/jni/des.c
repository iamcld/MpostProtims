...
static void function_des(int cryp_decrypt, unsigned char *DES_DATA, unsigned char *DESKEY, unsigned char *DES_RESULT);
...
static unsigned char LS[17] = { 0,1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1 };
static unsigned char Ki[17][49];
...
void Vect_Permutation(unsigned char *vect,unsigned char n_vect,
...
void function_des(int cryp_decrypt, unsigned char *DES_DATA,
                  unsigned char *DESKEY, unsigned char *DES_RESULT)
{
...
// input parameter��iMode    : ENCRYPT��DECRYPT -> ����
//                  TRI_ENCRYPT -> 3Des encrypt��TRI_DECRYPT -> 3Des decrypt
//          	 	psSource : source data
//           		psKey    : key
// ouput parameter��psResult : result
void vDes(int iMode, unsigned char *psSource, unsigned char *psKey, unsigned char *psResult)
{
	...
	case DES_ENCRYPT:
	case DES_DECRYPT:
		function_des(iMode, psSource, psKey, psResult);
		break;

	case TRI_ENCCLS:
		function_des(ENCRYPT, psSource, psKey, psResult);
		function_des(DECRYPT, psResult, psKey+8, sBuf);
		function_des(ENCRYPT, sBuf, psKey, psResult);
		break;

	case TRI_DECCLS:
		function_des(DECRYPT, psSource, psKey, psResult);
		function_des(ENCRYPT, psResult, psKey+8, sBuf);
		function_des(DECRYPT, sBuf, psKey, psResult);
		break;

	case TRI_ENCSTD:
		function_des(ENCRYPT, psSource, psKey, psResult);
		function_des(DECRYPT, psResult, psKey+8, sBuf);
		function_des(ENCRYPT, sBuf, psKey+16, psResult);
		break;

	case TRI_DECSTD:
		function_des(DECRYPT, psSource, psKey+16, psResult);
		function_des(ENCRYPT, psResult, psKey+8, sBuf);
		function_des(DECRYPT, sBuf, psKey, psResult);
		break;
	}
