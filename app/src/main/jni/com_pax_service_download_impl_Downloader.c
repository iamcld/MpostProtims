...
#include "com_pax_service_download_impl_Downloader.h"
#include "callback.h"
#include "mpos.h"
#include "protims.h"
#include "common.h"
...
#define FIELD_SIG_LISTENER				"Lcom/pax/service/download/IProgressListener;"
...
JNIEXPORT jint JNICALL Java_com_pax_service_download_impl_Downloader_download
  (JNIEnv *env, jobject obj){
	...
	LOGI("Terminal Information, iRet=%d", iRet);
	...
	LOGI("Terminal ExSN, iRet=%d", iRet);
...
{
	return ExecIntMethod(1, sgMdRecvTms, "java/lang/IllegalAceessException", "TMS : fail to receive data",
...
jint ExecIntMethod(unsigned char bRecv, jmethodID mdId, unsigned char *pszExcCls, unsigned char *pszExcMsg, unsigned char *psDataBuff, unsigned int iDataLen, int iOffset)
{
	...
	if(exc)
	{
		...
		if(pszExcCls && strcmp(pszExcCls, "")!=0 )
		{
		    LOGE("java/io/IOException 709");
			excClz = (*sgEnv)->FindClass(sgEnv, pszExcCls);
			LOGE("java/io/IOException 711");
		}
		else
		{
		     LOGE("java/io/IOException 714");
			excClz = (*sgEnv)->FindClass(sgEnv, "java/lang/IllegalAceessException");
			LOGE("java/io/IOException 717");
		}
		if(NULL == excClz) {
		LOGE("java/io/IOException excClz null");
			(*sgEnv)->DeleteLocalRef(sgEnv, data);
			return -1001;
		}
		if(pszExcMsg)
		{
		LOGE("null message ThrowNew java/io/IOException ");
			(*sgEnv)->ThrowNew(sgEnv, excClz, pszExcMsg);
		}
		else
		{
		    LOGE("ThrowNew java/io/IOException ");
			(*sgEnv)->ThrowNew(sgEnv, excClz, "ExecVoidMethod, no message->null");
		}
	}
...
int ExecStringGetMethod(jmethodID mdId, unsigned char *pszExcCls, unsigned char *pszExcMsg, unsigned char *psOutDataBuff, int iOffset, unsigned int iMaxLen, int *iOutLen)
{
	...
	int   iValLen = 0;
	jint iRet = 0;
	LOGE("ExecStringGetMethod befor");
...
LOGE("jni 794");
	pszRetVal = (*sgEnv)->GetStringUTFChars(sgEnv, sRetVal, NULL);
	if(NULL == pszRetVal)
	{
		*iOutLen = -1;
		return 0;
	...
	(*sgEnv)->ReleaseStringUTFChars(sgEnv, sRetVal, pszRetVal);
	pszRetVal = NULL;
	LOGE("ExecStringGetMethod end");
	return 0;
}

JNIEXPORT jint JNICALL Java_com_pax_service_download_impl_Downloader_getTermSN
  (JNIEnv * env, jobject obj, jbyteArray recvBuf){
    ...
    recvBuf = env->NewByteArray(8);
    jbyte *jby = env->GetByteArrayElements(recvBuf, 0);
    memcpy(jby, termSn, 8);
    env->SetByteArrayRegion(recvBuf, 0, 8, jby);
 ...
 JNIEXPORT jint JNICALL Java_com_pax_service_download_impl_Downloader_getTermExtSN
   (JNIEnv * env, jobject obj, jbyteArray recvBuf){
    char *pucRspDataOut;
    jint iRet;
    iRet = MposReadExSN(pucRspDataOut);
    return iRet ;
}
...
JNIEXPORT jint JNICALL Java_com_pax_service_download_impl_Downloader_getTerminalInfo
  (JNIEnv * env, jobject obj, jbyteArray recvBuf){
    ...
    if(iRet != 0 ){
     ...
     recvBuf = env->NewByteArray(30);
     jbyte *jby = env->GetByteArrayElements(recvBuf, 30);
     memcpy(jby, terInfo, 30);
     env->SetByteArrayRegion(recvBuf, 0, 30, jby);
